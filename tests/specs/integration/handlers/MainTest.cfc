	...
	function run(){
		...
		describe( "Main Handler", function(){
			...
			it( "+homepage renders", function(){
				...
				expect(	event.getValue( name="qItems", private=true ) ).toBeQuery();
			});

			it( "+completed renders", function(){
				var event = execute( event="main.completed", renderResults=true );
				expect(	event.getValue( name="qItems", private=true ) ).toBeQuery();
			});

			it( "+complete relocates", function(){
				var event = execute( event="main.complete" );
				expect(	event.getValue( "setnextevent_event", "" ) ).toBe( "main" );
			});

			it( "+add renders", function(){
				getRequestContext().setValue( name="description", value="integration test");
			...
			it( "+add validates", function(){
				

				var event = execute( event="main.add", renderResults=true );

				expect(	event.getValue( name="message", private=true ) ).toBe("Description is required!");
				expect(	event.getValue( name="qItems", private=true ) ).toBeQuery();
			});
